...
from src.deploy             import TCHECK, VS, DB
...
def deploy(config):
    ...
    if write_vs2DB() != True:
        return False
    logger.debug("# Deploy # Start all tchecks...")
...
def start_check():
    ...
    TCHECK['Has_Setup'] = 'True'
    return True

# result check, input: [('vs1', 1), ('vs2', 0)]
def check_db(vs_in):
    ...
    info = '# test # Result' + str(vs_in) + ' is expected, test sucessful!!!' 
    logger.debug(info)
